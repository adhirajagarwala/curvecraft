...
def test_run_mosfet_id_vgs_demo_creates_core_outputs_without_ngspice(
    tmp_path,
) -> None:  # type: ignore[no-untyped-def]
    result = run_mosfet_id_vgs_demo(
        output_dir=tmp_path,
        run_ngspice_if_available=False,
    )
...
def test_cli_mosfet_id_vgs_demo_accepts_temp_output_dir(
    tmp_path,
    capsys,
) -> None:  # type: ignore[no-untyped-def]
    exit_code = main(
